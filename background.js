...
動画URL: ${videoUrl}`;
    ...
    default:
      if (pageText) {
        // 一般的なウェブページ用のプロンプト (既存のものをベースに修正)
        prompt = `以下のウェブページのタイトルと本文を読んで、内容を600文字程度で日本語で要約してください。記事の主要なトピック、議論されている内容、そして結論を簡潔にまとめてください。\n\nタイトル: ${pageTitle}\n\n本文:\n${pageText}`;
      } else {
        // pageUrlはあるがpageTextがない場合 (例: PDFなど、本文抽出が困難なケースを想定)
        // もしくは siteType が 'general' 以外で予期せず pageText がない場合
        // ここでは、pageText がない general サイトはエラーとする
